...
let text = {
  ...
  description:
    "Postanowiłem usprawnić moją codzienną drogę do pracy, budowałem portal komunikacji miejskiej. Na portalu można wybrać przystanek i śledzić aktualny stan naszego pojazdu, aby uniknąć niepotrzebnego czekania na przystanku. Ponadto, istnieje możliwość dodania własnej trasy i dokładnej analizy dostępnych przystanków.",
  technology:
    "Do implementacji narzędzia wykorzystałem Reacta oraz .net .net api działa jako pośrednik który przyjmuje otwarte dane ztm gdańsk i przekształca je w łatwiejsze do analizowania . ",
  obstacles:
    "Otwarte dane ZTM Gdańsk nie są najlepszej jakości, dokumentacji w zasadzie nie ma i momentami wątpiłem czy projekt ma sens.",
  benefits:
    "Dojazd do pracy jest bardzo ułatwiony,a ja nie musze martwić się spóźnieniami.",
};

export default function SecondProject(language) {
  const url = language.language + "/ztmapp";
  ...
  const panes = [
    {
      menuItem: { icon: "file code", content: "Technologie" },
      render: () => <Tab.Pane>{text.technology}</Tab.Pane>,
    },
    {
      menuItem: { icon: "lock", content: "Wyzwania" },
      render: () => <Tab.Pane>{text.obstacles}</Tab.Pane>,
    },
    {
      menuItem: { icon: "dollar", content: "Benefity" },
      render: () => <Tab.Pane>{text.benefits}</Tab.Pane>,
    },
  ];
  return (
